...
class ThreadChatScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final threadCubit = ValueCubit<ThreadChatState>(const ThreadChatState());
    final replyController = TextEditingController();
    final scrollController = ScrollController();

    // Fetch initial thread replies via repository
    _fetchThreadReplies(context, threadCubit, scrollController);
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Column(
            ...
            children: [
              Text(patientName ?? 'Prescription Thread',
  ...
  Future<void> _fetchThreadReplies(
    BuildContext context,
    ValueCubit<ThreadChatState> threadCubit,
    ScrollController scrollController,
  ) async {
    try {
      final repo = context.read<SupabaseChatRepository>();
      final result = await repo.fetchThreadReplies(parentMessageId);
      final List<ChatMessage> replies = (result['replies'] as List<ChatMessage>?) ?? [];

      if (replies.isNotEmpty) {
        threadCubit.update(ThreadChatState(replies: replies, isLoading: false));
        _scrollToBottom(scrollController);
        return;
      }

      // Fallback fetch if empty thread replies
      final messages = await repo.fetchChatMessages(chatId);
      final threadReplies = messages      
          .where((m) => m.appointmentId == parentMessageId || m.id != parentMessageId)
          .toList();

      threadCubit.update(ThreadChatState(replies: threadReplies, isLoading: false));
      _scrollToBottom(scrollController);
    } catch (_) {
      threadCubit.update(ThreadChatState(replies: threadCubit.state.replies, isLoading: false));
    }
  }
  ...
  Future<void> _sendReply(
    ...
    try {
      ...
      final updatedList = List<ChatMessage>.from(threadCubit.state.replies)..add(replyMsg);
      threadCubit.update(threadCubit.state.copyWith(replies: updatedList));
      _scrollToBottom(scrollController);
